...
var googlePlusUserLoader = (function() {
  ...
  **/
  ...
  function removeItemCallback(containerId, key) {
  ...
  return {
    onload: function () {
     ...
     sampleSupport.addFlow( {
       name: '(key, value) storage',
       subflows: [
         { 
           name: 'Local storage',
           container: { id: '_localstorage' },
           side: {
             'actions': [ 
               {name: 'add', onclick: function() { addKeyValueItem('_localstorage')} },
               {name: 'clear all', onclick: function() { clearAll('_localstorage')} }
             ]
           }
         }
       ]
...
window.addEventListener('DOMContentLoaded', googlePlusUserLoader.onload);
